...
Sherpa-ONNX models are operator-installed under ignored `.runtime` directories
and are not part of the Git repository or default GitHub release artifact.
...
For a packaged Windows release build, `talk-desktop.exe` now resolves its
default config path in this order when `--config` is omitted:

1. `talk-desktop.toml` next to the executable, for release-side direct launch
2. `examples/dev-config.toml` under the current working directory, for repo
   development runs

Release packages generated by `Publish-TalkRelease.ps1` now include
`talk-desktop.toml` automatically, so a packaged `talk-desktop.exe` no longer
depends on the repo-only `examples/` tree just to start.

Packaged releases now also include:

```text
Start-TalkDesktop.ps1
Invoke-TalkDesktopLiveHotkeyProbe.ps1
Invoke-TalkDesktopQwenGlobalHotkeyProbe.ps1
Invoke-TalkDesktopQwenGlobalHotkeySoak.ps1
Invoke-TalkDesktopQwenNativeMicProbe.ps1
Invoke-TalkDesktopReleaseSmoke.ps1
```

That launcher resolves `talk-desktop.exe`, `talk-desktop.toml`, and
`TALK_PROVIDER_API_KEY` for release-side direct use. When the effective config
targets the packaged DashScope / Qwen path, it can also auto-discover the local
credential file at
`%USERPROFILE%\.neuro\qwen-platform\qwen-dashscope-openai\api-key\manual-live.json`
so you can launch the packaged shell and packaged Qwen probes without manually
retyping the key path on every machine. You can optionally pass a different
hotkey so you do not need to hand-edit the packaged TOML just to work around a
temporary shortcut conflict on the current machine. It also accepts:

- `-InputDevice '<name>'` to pin a specific Windows microphone / virtual input
  endpoint into the generated runtime launch config;
- `-ListInputDevices` to query the packaged `talk.exe readiness --json` path
  and print the Talk-visible native input-device inventory before launch;
- `-ProbeAudio` with optional `-ProbeSeconds <n>` to run the packaged
  `talk.exe probe-audio` path against the effective config and return signal
  metrics like selected input device, duration, peak, and RMS before launch.
- `-ProbeQwenRoundTrip` with `-ApiKey` / `-ApiKeyJsonPath` to capture live
  native audio first and, only when the signal is non-silent, feed that WAV
  into `talk.exe once --audio-file` against the packaged Qwen-compatible
  provider path.
...
`talk-desktop.exe` is the first Windows-first OpenLess / Typeless style Talk
shell. It is intentionally small:
...
`talk-desktop.exe` now stays alive in tray-only mode instead of exiting. The
...
`talk-desktop.exe` also stays alive in a config-unavailable tray mode. In that
...
Packaged releases now also include `talk-desktop.toml` as a support file. This
is the default desktop config template for direct release-side launch and is
recorded in the release manifest under `supportFiles`.

Packaged releases now also include `Start-TalkDesktop.ps1` as a support file.
This launcher is the default release-side helper for starting the desktop shell
with either an explicit provider API key or the auto-discovered packaged Qwen
credential path and, when needed, a temporary hotkey override.

## Desktop release packaging automation

`scripts/Publish-TalkRelease.ps1` is the release-side wrapper for Talk. It:

- reruns the Talk workspace verification commands unless `-SkipVerification` is used;
- rebuilds `talk.exe` and `talk-desktop.exe` unless `-SkipBuild` is used;
- copies both binaries into `release/Talk/<version-id>/`;
- reruns the scripted desktop release smoke unless `-SkipSmoke` is used;
- reruns explicit `native_windows` backend preflight unless `-SkipNativePreflight` is used;
- writes `BUILD_INFO.txt` with the exact smoke scenarios and evidence paths;
- writes `manifest.json` with release metadata, command history, exe hashes, and
  build log references, plus `nativePreflight` evidence;
- writes `release-summary.json` with a concise consumer-facing summary of
  verification, desktop smoke, native preflight, and native readiness status;
- writes `logs/talk-release-*.log` with the executed verification/build command,
  working directory, exit code, and captured output;
- writes `checksums.sha256` for every file in the version directory except the
...
Release archives expose the user-facing executable as `talk.exe` under
`release\Talk\<versionId>\`. The Cargo binary is also `talk`, so the release
copy no longer performs an old-name translation.
